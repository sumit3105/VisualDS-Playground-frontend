...
import { getPlaygrounds, addPlayground, updatePlayground } from "../services/playgroundService";
import toast from "react-hot-toast";
import PlaygroundCard from "../components/PlaygroundCard"; // assuming this exists

export default function ProfilePage() {
  const navigate = useNavigate();
  const [profile, setProfile] = useState({ name: "", email: "", imageUrl: "", id: null });
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      setMyPlaygrounds(myPlaygrounds.map(pg => 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const updatedUser = await updateUserProfile(selectedImage, profile.name);
  ...
  const renderProfileImage = () => {
    if (previewUrl) {
      ...
      return <img src={profile.imageUrl} alt="Profile" className="w-full h-full object-cover" />;
    } else {
